...
public class CalculationUtils {
    ...
    public static double[] performCalculation(Context context, @CalculatorType int type,
        ...
        switch (type) {
            ...
            case CalculatorType.DAILY:
                return calculateDaily();
            case CalculatorType.WEEKLY:
                return calculateWeekly();
            case CalculatorType.BIWEEKLY:
                return calculateBiweekly();
            case CalculatorType.MONTHLY:
                return calculateMonthly();
            case CalculatorType.YEARLY:
    ...
    /**
     * Checks if the hours are valid. If they are, then -1 will be returned. If not, a string resource ID will be
    ...
    public static int checkIfHoursAreValid(double hours, @CalculatorType int type){
        switch (type) {
            case CalculatorType.HOURLY:
                if (hours > MAX_WEEKLY) {
                    return R.string.fun_error_hourly_weeks;
                }
                break;
            case CalculatorType.DAILY:
                if (hours > MAX_DAILY) {
                    return R.string.fun_error_hourly_days;
                }
                break;
            case CalculatorType.WEEKLY:
                if (hours > MAX_WEEKLY) {
                    return R.string.fun_error_weekly;
                }
                break;
            case CalculatorType.BIWEEKLY:
